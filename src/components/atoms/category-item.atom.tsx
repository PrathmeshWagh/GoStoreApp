...
const CategoryItem = (props: CategoryItemProps) => {
	...
	const { price, mrp, image, title, containerStyles, parentCategory, productId, supplierId } =
		props;
	const discount = getDiscount(price, mrp);
	const { colors } = useTheme();

	const handlePress = () => {
		let url = `/category/${parentCategory}/${formatUrl(
			title
		)}?id=${productId}&supplierId=${supplierId}`;
		dispatch(
			openModal({
				view: 'loading',
				title: 'Redirecting...'
		...
		<TouchableRipple onPress={handlePress}>
			<View style={[styles.item, containerStyles]}>
				<View style={[styles.imageContainer]}>
					<FastImages url={image} style={[styles.image]} mode="contain" />
				</View>
				<Text numberOfLines={2} variant="titleMedium" style={[styles.title]}>
					{title}
				</Text>
				<Rupee money={price} styles={[styles.price]} />
				<View style={[styles.mrpWrapper]}>
					<Rupee money={mrp} styles={[styles.mrp]} />
					<Text
						numberOfLines={2}
						variant="titleMedium"
